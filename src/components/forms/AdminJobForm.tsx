import { adminCreateJobAction } from "@/lib/actions/platform";
...
export function AdminJobForm() {
  ...
  return (
    <form action={createJob} className="card-pad grid md:grid-cols-2 gap-4">
      ...
      <Field name="companyNameText" label="اسم الشركة النصي" />
      ...
      <div className="md:col-span-2 flex flex-wrap gap-4 text-sm">
        <label><input type="checkbox" name="featured" /> مميزة</label><label><input type="checkbox" name="urgent" /> عاجلة</label><label><input type="checkbox" name="pinned" /> مثبتة</label><label><input type="checkbox" name="womenFriendly" /> مناسبة للسيدات</label><label><input type="checkbox" name="noExperienceRequired" /> بدون خبرة</label><label><input type="checkbox" name="requiresDrivingLicense" /> تتطلب رخصة</label>
      </div>
...
function Field({ name, label, type = "text", required }: { name: string; label: string; type?: string; required?: boolean }) { return <div><label className="label">{label}</label><input className="input" name={name} type={type} required={required} /></div>; }
