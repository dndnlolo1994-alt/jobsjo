    ...
    if (employerEmail) {
      try {
        await sendNewApplicationAlert({
          ...
          jobTitle: job.title,
          applicantName: cv?.fullName || user.fullName,
          applicantPhone: seeker?.phone || null,
          applicantEmail: user.email,
          applicantCity: seeker?.city || cv?.city || null,
          applicantHeadline: seeker?.headline || cv?.jobTitle || null,
          applicantSummary: seeker?.summary || cv?.summary || null,
          skills: seeker?.skills || cv?.skills?.map?.((s: any) => s.name).join(",") || null,
          educationLevel: seeker?.educationLevel || null,
