...
export default async function CompanyPage({ params }: { params: Promise<{ slug: string }> }) {
  async function claimAction(formData: FormData) {
    "use server";
    await createClaimAction(null, formData);
  }
  const { slug } = await params;
  const company = await prisma.company.findUnique({ where: { slug }, include: { jobs: { where: { status: "PUBLISHED" }, include: { company: { select: { name: true, slug: true, logoUrl: true } } }, orderBy: { publishedAt: "desc" } } } });
  if (!company) notFound();
